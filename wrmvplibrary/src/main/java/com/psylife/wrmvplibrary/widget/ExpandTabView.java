...
public class ExpandTabView extends LinearLayout implements OnDismissListener {
	...
	public void setValue(ArrayList<String> textArray, ArrayList<View> viewArray) {
		...
		for (int i = 0; i < viewArray.size(); i++) {
			...
			tButton.setSingleLine();
			addView(tButton);
			View line = new TextView(mContext);
			line.setBackgroundResource(R.color.color_line);
			if (i < viewArray.size() - 1) {
				LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(1, LinearLayout.LayoutParams.MATCH_PARENT);
				lp.gravity = Gravity.CENTER_VERTICAL;
				lp.setMargins(0,mContext.getResources().getDimensionPixelOffset(R.dimen.line_m_top),0,mContext.getResources().getDimensionPixelOffset(R.dimen.line_m_down));
				addView(line, lp);
			}
